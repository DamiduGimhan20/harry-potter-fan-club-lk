// src/components/AdminTheoryForm.tsx
...
export function AdminTheoryForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 bg-slate-800 p-6 rounded-xl border border-slate-700">
      ...
      <div className="space-y-2">
        <label className="block text-sm font-medium text-slate-300">Theory Content</label>
        <textarea
          required
          rows={8}
      ...
      <div className="flex justify-end space-x-4 pt-4">
        ...
        >
          ...
          <span>{uploading ? 'Uploading...' : initialData ? 'Update Theory' : 'Publish Theory'}</span>
        </button>
      </div>
    </form>
  );
